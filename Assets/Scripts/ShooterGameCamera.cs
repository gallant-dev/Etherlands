...
public class ShooterGameCamera : MonoBehaviour {
	...
	public Vector3 camOffset   = new Vector3(0f, 0.7f, -3.4f); // offset of camera from pivotOffset (?) ben0bi
	...
	void LateUpdate () {
		...
		if (Physics.Raycast(cam.position, cam.forward, out hit, 100, mask)) {
			...
			aimTargetDist = Mathf.Max((charStats.GetStat("primaryAttackRangeBase") * charStats.GetStat("primaryAttackRangeMultiplier")), prevDist);
		}
